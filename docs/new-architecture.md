...
*(Status: scaffold exists and builds/lints clean. `analysis/kwic` is real and Playwright-verified end to end (Phase 3). Every other page is still a stub with no real data — see the KUIS-FE repo's own README for the per-page dependency table.)*
...
- **End-to-end**: from KUIS-FE, log in, select a corpus, run KWIC — confirm results match the legacy Django page. ✅ done — a real headless-browser (Playwright) run, not a manual check: login, corpus selection, phrase search matching Phase 2's parity-checked count exactly, a shareable URL that actually replays the search on reload, corrected-mode search, and a reviewed screenshot. Zero CORS errors in the browser's own console.
