...
**Not done from Phase 4's scope:** `FrequencyRepository`/`CollocationRepository`/`NgramRepository`'s interfaces are the only ones from Phase 2's provisional five now proven against real data — `ErrorAnalyticsRepository` remains entirely unimplemented (Phase 6, blocked on the taxonomy). Tier 1/2 schema optimization (Phase 5) hasn't started — these three repositories all still query the raw `Token.word` column. Nothing is committed to git in either repo.
...
*(Status: the tree above exists, fully populated for KWIC/Frequency/Collocation/Ngram — `repositories/postgres/`, `services/`, `schemas/`, `routers/` all written and parity-tested (Phase 2 for KWIC, Phase 4 for the other three). `repositories/base.py` defines all 5 ABCs; `ErrorAnalyticsRepository` is the only one with no concrete implementation yet — Phase 6. `worker/tasks/*` is still empty — Phase 5.)*
