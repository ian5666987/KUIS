...
| **KWIC (Legacy)** | registered | Concordance search over a live XML parse; word or phrase, context window 2–10, sortable by left/right/file, paginated, each line attributed to its file | `main/views.py` (`kwic`, `_kwic_results`), `main/templates/main/kwic.html` |
| **KWIC (Fast)** | registered | Same concordance view for a single word form, answered from the `Token` table instead of re-parsing; one query per document on the current page | `main/views.py` (`kwic_search`, `_word_index_matches`, `_hydrate_word_index`), `main/models.py` `Token`, `main/templates/main/kwic_search.html` |
